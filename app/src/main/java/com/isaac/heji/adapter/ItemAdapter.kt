...
class ItemAdapter(data: MutableList<ItemInfo>?) :
    ...
    override fun convert(holder: BaseViewHolder, item: ItemInfo) {
        ...
        if (item.selected) {
            holder.setBackgroundRes(R.id.value, itemBackground[item.style % 3][0])
            holder.setTextColor(R.id.value, ContextCompat.getColor(mContext, R.color.white))
        } else {
            holder.setBackgroundRes(R.id.value, itemBackground[item.style % 3][1])
            holder.setTextColor(R.id.value, ContextCompat.getColor(mContext, itemTextColor[item.style % 3]))
        }
